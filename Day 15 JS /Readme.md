...
This file contains various JavaScript assignments from Sheriyans Coding School,
covering topics like conditionals, loops, string manipulation, and more.
...
if (
  (num1Val < num2Val && num1Val > num3Val) ||
  (num1Val < num3Val && num1Val > num2Val)
) {
  console.log(num1Val, "is the second largest value");
} else if (
  (num2Val < num3Val && num2Val > num1Val) ||
  (num2Val < num1Val && num2Val > num3Val)
) {
  console.log(num2Val, "is the second largest value");
} else if (
  (num3Val < num1Val && num3Val > num2Val) ||
  (num3Val < num2Val && num3Val > num1Val)
) {
...
1. **Save** the above content as a single JavaScript file (e.g., `Main.js`).
...
4. **Note:** Some sections require user input via `prompt()`.
   - Sections containing syntax errors (if any) are commented out.
